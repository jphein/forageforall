/**
 * Web schema — identical to schema.ts but imports `i` from @instantdb/react
 * so the native package (which pulls in react-native internals) is never
 * bundled during `expo export --platform web`. Metro resolves .web.ts first.
 */
